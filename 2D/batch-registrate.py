...
def main(config):
    ...
    for dirpath, dirnames, filenames in os.walk(moving_data_dir):
        ...
        for segname in [f for f in filenames if f.endswith("seg24.nii.gz")]:    
    ...
    for dirpath, dirnames, filenames in os.walk(fixed_data_dir):
        ...
        for filename in [f for f in filenames if f.endswith("seg24.nii.gz")]:
        ...
        for fixed_set_name,fixed_mri,fixed_seg in zip(fixed_set_name_arr,fixed_files_paths_mri,fixed_file_paths_seg):
            if moving_mri != fixed_mri:
                ...
                if not os.path.isdir(savedir):
                    os.makedirs(savedir)
                if config.visuals:
                    savedirvis = savedir + '/' + 'visuals'
                    if not os.path.isdir(savedirvis):
                        os.makedirs(savedirvis)
                avg_dice, runtime_run, mean_neg_j_run, ratio_neg_j_run = Registration.main(config = config, moving_mri = moving_mri, fixed_mri = fixed_mri,savedir=savedir, fixed_seg_in = fixed_seg, moving_seg_in=moving_seg, save_visuals=config.visuals)
